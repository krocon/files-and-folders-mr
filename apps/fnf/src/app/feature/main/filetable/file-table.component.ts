...
import {QueueNotifyEventIf} from "../../task/domain/queue-notify-event.if";
...
import {getButtonEnableStates, PackDialogData, PackDialogResultData, DownloadDialogData, DownloadDialogResultData} from "@fnf-data";
import {PanelManagementService} from "../../../service/panel/panel-management-service";
import {PackDialogService} from "../../cmd/pack/pack-dialog.service";
import {DownloadDialogService} from "../../cmd/download/download-dialog.service";
...
export class FileTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    private readonly packDialogService: PackDialogService,
    private readonly downloadDialogService: DownloadDialogService,
  ) {
  ...
  async actionCall(action: string) {
    ...
    } else if (action === "OPEN_PACK_DLG") {
      this.openPackDlg();
      
    } else if (action === "OPEN_DOWNLOAD_DLG") {
      this.openDownloadDlg();
  ...
  openPackDlg() {
    ...
    if (!selectedRows || selectedRows.length === 0) {
      return; // skip if no selection
    }

    const activeTabOnActivePanel = this.appService.getActiveTabOnActivePanel();
    const dir = this.dirPara?.path ?? activeTabOnActivePanel.path;
    
    // Convert selected rows to source paths
    const sourcePaths = selectedRows.map(item => item.dir + '/' + item.base);
    
    const data = new PackDialogData(
      sourcePaths,
      "", // targetFilename - will be generated by dialog
      dir, // targetDirectory
      "", // password
      "7z", // format
      5, // compressionLevel
      this.panelManagementService.getDirsFromAllTabs()
    );

    this.packDialogService.open(data, (result: PackDialogResultData | undefined) => {
      if (result) {
        // Convert result to FilePara format for the backend
        const filePara = {
          cmd: 'pack' as const,
          source: selectedRows,
          target: {
            dir: result.targetDirectory,
            base: result.targetFilename,
            ext: result.targetFilename.split('.').pop() || ''
          },
          password: result.password,
          format: result.format,
          compressionLevel: result.compressionLevel
        };
        this.actionExecutionService.callActionPack(filePara);
      }
    });
  }
  ...
  openDownloadDlg() {
    ...
    const data = new DownloadDialogData(
      sourcePaths,
      "", // targetFilename - will be generated by dialog
      dir, // targetDirectory
      "", // password
        ...
        import("../../cmd/download/download.service").then(module => {
          ...
          downloadService
            ...
            (blob: Blob) => {
              ...
              link.href = url;
              link.download = result.targetFilename;
